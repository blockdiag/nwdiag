...
def parse_option():
    version = "%%prog %s" % rackdiag.__version__
    ...
    configpath = options.config or "%s/.rackdiagrc" % os.environ.get('HOME')
    if os.path.isfile(configpath):
        config = SafeConfigParser()
        config.read(configpath)

        if config.has_option('rackdiag', 'fontpath'):
            fontpath = config.get('rackdiag', 'fontpath')
